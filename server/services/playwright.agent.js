...
async function discoverByClicking(page, landingUrl, routeSet, visitedSet, profile, onLog) {
  ...
  for (const sel of profile.navSelectors) {
    try {
      const count = await page.locator(sel).count();
      for (let i = 0; i < Math.min(count, 30); i++) navHandles.push({ sel, idx: i });
    } catch {}
  }

  onLog('info', `  ${navHandles.length} nav elements from ${profile.name} profile — clicking…`);

  const clickedTexts = new Set();
  let newRoutes = 0;

  for (const { sel, idx } of navHandles.slice(0, 120)) {
    try {
      ...
      // Wait using profile's preferred strategy
      ...
      if (routeSet.size > sizeBefore) newRoutes += (routeSet.size - sizeBefore);

      // Navigate back to landing (authenticated) — NOT baseUrl
      ...
      if (curPath !== landingPath) {
        ...
        await page.waitForTimeout(400);
        // Re-expand collapsed menus (Angular may re-render on navigation)
        await expandCollapsedMenus(page, profile, () => {}).catch(() => {});
      }
    } catch {}
  }

  onLog('info', `  ↳ Nav click scan complete — ${newRoutes} new routes captured`);
}
